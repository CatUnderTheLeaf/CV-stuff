...
def show_all_images(images, columns = 4):
    ...
    for img in images:
        ...
        plt.imshow(img)
        i+=1
    plt.savefig(os.path.dirname(full_path)+'/checkboards.png', bbox_inches='tight')
...
def show_images(orig_img, corr_img, xlabel1, xlabel2, cmap1 = None, cmap2 = None, save = False):
    ...
    plt.yticks([], [])
    if save:
        plt.savefig(os.path.dirname(full_path) + '/' + xlabel2.replace(" ", "_") + '.png', bbox_inches='tight')
    plt.show()
...
dist_test_img = plt.imread(images[6])
